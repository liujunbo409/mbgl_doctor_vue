    ...
    export default {
        ...
        methods: {
            ...
            getMsgVcCode() {
                ...
                if (this.getMsgVcCodeTimeoutKey !== 0) {
                    return
                }   // 60秒限制中点击无效
            ...
            register() {
                ...
                if (this.resetPsd) {
                    ...
                    this.$store.dispatch('user/register', requestBody).then(() => {
                        this.$bus.$emit('vux.toast', {type: 'success', text: '注册成功'});
                        this.$store.dispatch('hospList/load');
                        this.$store.dispatch('user/editStatus/get');
                        this.$toView('my/info');
                    }).catch(e => {
                        let text = e.timeout ? '网络错误' : e.message;
                        this.$bus.emit('vux.toast', {type: 'cancel', text});
                    });
                }
            },
        }
