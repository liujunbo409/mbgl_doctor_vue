...
<script>
    import base64Upload from "../../util/request/base64Upload";
    ...
    export default {
         ...
         */
        props: ["qrUrl", "qrSize", "qrText", "qrLogo", "qrLogoSize", "qrTextSize", "user_id"],
        created() {
        },
        mounted() {
            this.getQiniuBase64ImgUrl();
        },

        methods: {
            getQiniuBase64ImgUrl() {
                ...
                QRCode.toDataURL(this.qrUrl, {errorCorrectionLevel: "H"}, (err, url) => {
                    ...
                    setTimeout(() => {
                        ...
                        base64_url = canvas.toDataURL().slice(canvas.toDataURL().indexOf(',') + 1);
                        // if (!this.$store.state.user.userInfo.qr_url) {
                            base64Upload(base64_url).then(res => {
                                _request({
                                    url: 'my/updateById',
                                    method: 'post',
                                    data: {
